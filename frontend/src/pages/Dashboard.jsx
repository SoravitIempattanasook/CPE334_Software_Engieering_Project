...
/**
 * NOTE:
 * - หน้านี้ไม่ห่อด้วย <SidebarLayout> เพื่อเลี่ยง sidebar ซ้อน
 * - ให้มี Sidebar ที่ root layout เพียงชั้นเดียว
 */
...
function Dashboard() {
  ...
  const loadUserEvents = async () => {
    ...
    const { data, error } = await supabase
      .from("Activity")
      .select(
        "id, name, description, start_time, end_time, all_day, ActivityType(color, name)"
      )
  ...
  const todayEvents = useMemo(
    () =>
      events.filter((e) =>
        dayjs(e.start).isBetween(startOfToday, endOfToday, null, "[]")
      ),
    [events]
  );
  ...
  const overdue = useMemo(
    () => events.filter((e) => dayjs(e.end).isBefore(now)),
    [events]
  );
  ...
  const kpi = useMemo(() => {
    const thisWeek = events.filter(
      (e) =>
        dayjs(e.start).isBefore(endOfWeek) &&
        dayjs(e.end).isAfter(now.startOf("week"))
    ).length;
  ...
  const renderItem = (item) => (
    <List.Item>
      <List.Item.Meta
        title={
          <span>
            <Tag color={item.color} style={{ color: "#fff" }}>
              {item.typeName}
            </Tag>
            {item.title}
          </span>
        ...
        description={
          <span>
            ...
            <br />
            {item.description || (
              <em style={{ color: "#888" }}>ไม่มีรายละเอียด</em>
            )}
          </span>
        }
      />
  ...
  return (
    <div style={{ padding: 20 }}>
      <Row justify="space-between" align="middle" style={{ marginBottom: 12 }}>
        <Col>
          ...
          <div style={{ color: "#6b7280" }}>
            สรุปกิจกรรมและเตือนความจำสำหรับนักศึกษา
          </div>
        </Col>
        {/* 🔻 ลบปุ่มเพิ่มกิจกรรมด่วนออกแล้ว */}
      </Row>
      ...
      <Row gutter={[16, 16]}>
        ...
        <Col xs={12} sm={12} md={6}>
          <Card>
            <Statistic title="ชั่วโมงรวมวันนี้" value={kpi.hoursToday} suffix="ชม." />
          </Card>
        </Col>
        <Col xs={12} sm={12} md={6}>
          <Card>
            <Statistic title="สัปดาห์นี้" value={kpi.thisWeek} />
          </Card>
        </Col>
        <Col xs={12} sm={12} md={6}>
          <Card>
            <Statistic title="ทั้งหมด" value={kpi.total} />
          </Card>
        </Col>
      </Row>
      ...
      <Row gutter={[16, 16]} style={{ marginTop: 16 }}>
        <Col xs={24}>
          <Card
            title={
              <span>
                <ExclamationCircleOutlined /> ค้าง/เลยกำหนด
              </span>
            }
          >
            {overdue.length ? (
